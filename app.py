...
def get_latest_df():
    """구글 시트에서 최신 데이터를 읽어옵니다."""
    conn = st.connection("gsheets", type=GSheetsConnection)
    try:
        # ttl="0s"로 설정하여 캐시 없이 실시간 데이터를 가져옵니다.
        return conn.read(spreadsheet=SHEET_URL, ttl="0s")
    except:
        # 시트가 비어있을 경우 기본 스키마 생성
    ...
    for r in ["1번 스터디룸", "2번 스터디룸"]:
        with st.expander(f"🚪 {r}", expanded=True):
            ...
            if not occ.empty:
                ...
                st.markdown(f"<p style='font-size: 1.1rem; font-weight: bold;'>⏰ 종료 예정: {current_user['종료']}</p>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    if not st.session_state.reserve_success:
        ...
        if not available_start: st.error("⚠️ 오늘은 더 이상 예약 가능한 시간이 없습니다.")
            ...
            # 버튼 비활성화 조건: 이름 입력 AND 학번 10자리 AND 사진 업로드
...
with tabs[3]:
    ...
    if st.button("연장 가능 여부 확인", key="btn_ext_check"):
        ...
        if not res_e.empty:
            target = res_e.iloc[-1]
            if target["출석"] != "입실완료": st.error("🚫 먼저 QR 인증을 해주세요.")
            else:
                ...
                else: st.warning("⚠️ 이용 종료 30분 전부터 가능합니다.")
    ...
    if 'ext_target' in st.session_state:
        ...
        if not new_en_opts: st.warning("이후 예약으로 인해 연장 불가")
        else:
            new_en = st.selectbox("새로운 종료 시각", new_en_opts, key="ext_sel")
...
with st.expander("🛠️ 관리자 전용 메뉴"):
    ...
    if pw == "bio1234":
        st.write("### 📋 전체 예약 명단 (구글 시트 동기화)")
        st.dataframe(df_all, use_container_width=True)
        if not df_all.empty:
            st.divider()
            st.write("### 🔍 학생증 사진 확인")
            labels = [f"{r['이름']} | {r['날짜']} | {r['방번호']}" for _, r in df_all.iterrows()]
            sel = st.selectbox("대상을 선택하세요", range(len(labels)), format_func=lambda x: labels[x])
            ...
            else: st.warning("⚠️ 서버 재시작으로 사진 파일이 삭제되었습니다.")
