...
# --- [3. 사이드바 실시간 현황] ---
with st.sidebar:
    st.markdown("<h2 style='color:#3E7D6B;'>📊 실시간 예약 현황</h2>", unsafe_allow_html=True)
...
with tabs[0]:
    ...
    reg_date = c_date.date_input("📅 예약 날짜", min_value=now_kst.date(), max_value=now_kst.date()+timedelta(days=13), key="main_reg_date")
    reg_room = c_room.selectbox("🚪 스터디룸 선택", ["1번 스터디룸", "2번 스터디룸"], key="main_reg_room")

    if reg_date == now_kst.date():
    ...
    if not available_start_times:
        ...
        if st.button("🚀 예약 신청하기", key="btn_reservation"):
            ...
            elif check_overlap(reg_date, st_t, en_t, reg_room): st.error("❌ 이미 예약이 있습니다.")
            else:
                new_row = pd.DataFrame([[dept, name.strip(), sid.strip(), count, str(reg_date), st_t, en_t, reg_room, "미입실"]], columns=["학과", "이름", "학번", "인원", "날짜", "시작", "종료", "방번호", "출석"])
...
with tabs[3]:
    ...
    if st.button("연장 가능 여부 확인", key="btn_ext_check"):
        ...
        if not res_e.empty:
            ...
            else: st.warning(f"연장은 종료 30분 전부터 가능합니다.")
