...
def get_kst_now():
    """서버 시간을 한국 시간(KST)으로 변환"""
    return datetime.now(timezone.utc) + timedelta(hours=9)

def get_latest_df():
    """구글 시트에서 최신 데이터를 읽어옴 (제공된 컬럼 기준)"""
    conn = st.connection("gsheets", type=GSheetsConnection)
    try:
        # 캐시 없이 실시간 데이터를 가져오기 위해 ttl="0s" 설정
        return conn.read(spreadsheet=SHEET_URL, ttl="0s")
    except:
        # 시트가 비어있거나 접근 오류 시 기본 컬럼 생성
    ...
    for r in ["1번 스터디룸", "2번 스터디룸"]:
        with st.expander(f"🚪 {r}", expanded=True):
            ...
            if not occ.empty:
                ...
                st.markdown(f'<h3 style="color:{status_color}; margin-bottom: 5px;">{status_text}</h3>', unsafe_allow_html=True)
            ...
            st.markdown("<p style='font-size: 0.9rem; font-weight: bold;'>📅 다음 예약 안내</p>", unsafe_allow_html=True)
            if not next_res.empty:
                for _, row in next_res.iterrows(): st.caption(f"🕒 {row['시작']} ~ {row['종료']} (예약 완료)")
            else: st.caption("이후 예약 없음")
...
with tabs[0]:
    ...
    if not st.session_state.reserve_success:
        ...
        id_file = st.file_uploader("💳 학생증 사진 업로드 (본인 확인용)", type=['png', 'jpg', 'jpeg'])
        ...
        if not available_start: st.error("⚠️ 오늘은 더 이상 예약 가능한 시간이 없습니다.")
            ...
            if st.button("🚀 예약 신청", disabled=submit_disabled):
                ...
                if duration > timedelta(hours=3): st.error("🚫 최대 이용 가능 시간은 3시간입니다.")
                    ...
                    df_all = pd.concat([df_all, pd.DataFrame([new_data], columns=df_all.columns)], ignore_index=True)
                    update_gsheet(df_all)
...
with tabs[2]:
    if not df_all.empty:
        ...
        for r_n in ["1번 스터디룸", "2번 스터디룸"]:
            ...
            if room_day.empty: st.caption("예약 내역 없음")
            else:
...
with tabs[3]:
    ...
    if st.button("연장 가능 여부 확인"):
        ...
        if not res_e.empty:
            target = res_e.iloc[-1]
            if target["출석"] != "입실완료": st.error("🚫 먼저 QR 인증을 완료해야 연장이 가능합니다.")
            else:
                ...
                else: st.warning("⚠️ 이용 종료 30분 전부터 연장 신청이 가능합니다.")
        else: st.error("🔍 오늘 예약 내역을 찾을 수 없습니다.")
    if 'ext_target' in st.session_state:
        target = st.session_state['ext_target']
        new_en_opts = [t for t in time_options_all if t > target['종료']][:4]
        if not new_en_opts: st.warning("다음 예약으로 인해 연장이 불가능합니다.")
        else:
            new_en = st.selectbox("변경할 종료 시각", new_en_opts, key="ext_sel")
            if st.button("최종 연장 확정"):
                ...
                update_gsheet(df_all)
                st.success(f"✨ {new_en}으로 연장되었습니다."); del st.session_state['ext_target']; st.rerun()
...
with tabs[4]:
    ...
    if 'cancel_list' in st.session_state:
        ...
        target_idx = st.selectbox("취소할 예약을 선택하세요", range(len(opts)), format_func=lambda x: opts[x])
        if st.button("선택 예약 취소"):
            ...
            st.success("취소되었습니다."); del st.session_state['cancel_list']; st.rerun()
...
with st.expander("🛠️ 관리자 전용 메뉴"):
    ...
    if pw == "bio1234":
        ...
        if not df_all.empty:
            st.divider()
            st.write("### 🔍 학생증 사진 확인")
            labels = [f"{r['이름']} | {r['날짜']} | {r['방번호']}" for _, r in df_all.iterrows()]
            sel = st.selectbox("학생 선택", range(len(labels)), format_func=lambda x: labels[x])
            target_row = df_all.iloc[sel]
            img_path = os.path.join(IMG_DIR, str(target_row['사진파일명']))
            if os.path.exists(img_path): st.image(img_path, caption=f"{target_row['이름']} 학생증", width=400)
            else: st.warning("⚠️ 서버 재시작으로 사진이 삭제되었습니다.")
            if st.button("선택 예약 강제 삭제"):
                df_all = df_all.drop(df_all.index[sel])
                update_gsheet(df_all); st.success("삭제 완료"); st.rerun()
