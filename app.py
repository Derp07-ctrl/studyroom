    ...
    for idx, row in df.iterrows():
        if row["날짜"] == now_date and row["출석"] == "미입실":
            try:
                ...
                # 예약 시작 15분이 지났거나, 연장 후 재인증 안 했을 경우
...
with tabs[3]:
    ...
    if st.button("연장 가능 여부 확인", key="btn_ext_check"):
        ...
        if not res_e.empty:
            ...
            else: st.warning("연장은 종료 30분 전부터 종료 시각까지만 가능합니다.")
    ...
    if 'ext_target' in st.session_state:
        ...
        st.info("⚠️ 연장 확정 시 15분 이내에 QR 코드를 다시 스캔해야 예약이 취소되지 않습니다.")
        if st.button("연장 확정", key="btn_ext_confirm"):
            ...
            df_up.loc[idx, "출석"] = "미입실" # 재인증을 위해 상태 변경
...
with tabs[4]:
    ...
    if st.button("예약 조회하기", key="btn_can_lookup"):
...
with st.expander("🛠️ 관리자 전용 메뉴"):
    ...
    if pw == "bio1234":
        ...
        if not df_ad.empty:
            ...
            sel = st.selectbox("강제 삭제 대상", range(len(labels)), format_func=lambda x: labels[x])
            if st.button("강제 퇴실/삭제 실행"):
                t = df_ad.iloc[sel]
