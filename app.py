...
def process_qr_checkin(df):
    ...
    if "checkin" in q_params:
        ...
        # 입실 가능 시간 설정 (예약 10분 전부터 예약 종료 전까지)
        early_limit = (now_kst + timedelta(minutes=10)).strftime("%H:%M")
        
        # 조건: 방번호 일치 + 날짜 일치 + 시작시간 10분 전부터 + 종료시간 전 + 미입실 상태
...
st.markdown("""
    ...
    .res-card { padding: 15px; border-radius: 12px; border-left: 6px solid var(--point-color); background-color: rgba(167, 215, 197, 0.1); margin-bottom: 12px; }
    </style>
...
with tabs[0]:
    ...
    if not st.session_state.reserve_success:
        ...
        if st.button("🚀 예약 신청하기", key="btn_reservation"):
            ...
            if not is_ready: st.error("⚠️ 모든 정보를 정확히 입력해주세요.")
            else:
                ...
                if check_overlap(sel_date, st_t, en_t, room): st.error("❌ 이미 예약된 시간입니다."); st.stop()
        ...
        st.success("🎉 예약 완료!"); if st.button("처음으로"): st.session_state.reserve_success = False; st.rerun()

with tabs[1]:
    st.markdown('<div class="step-header">🔍 내 예약 확인 (대표자/구성원 공통)</div>', unsafe_allow_html=True)
    mc1, mc2 = st.columns(2)
    m_n, m_s = mc1.text_input("조회할 이름", key="lookup_n"), mc2.text_input("조회할 학번 (10자리)", key="lookup_s")
    if st.button("조회하기"):
    ...
    if pw == "bio1234":
        ...
        with admin_tab2:
            if os.path.exists(HISTORY_FILE):
                ...
                st.download_button("📥 기록 다운로드", data=df_hist.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig'), file_name="history.csv", mime="text/csv")
