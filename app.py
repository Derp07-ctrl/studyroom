    ...
    if not available_start:
        ...
        if st.button("🚀 예약 신청", key="btn_reservation"):
            if not (name.strip() and sid.strip()):
                st.error("⚠️ 이름과 학번을 모두 입력해 주세요.")
            elif is_already_booked(name, sid):
                st.error("🚫 이미 등록된 예약 내역이 존재합니다. (1인 1예약)")
            elif check_overlap(date, st_t, en_t, room):
                st.error("❌ 선택하신 시간에 이미 다른 예약이 있습니다.")
            else:
                ...
                st.markdown(f"""
                    <div class="success-receipt">
                        ...
                        <div class="receipt-item"><span>신청자</span><b>{name.strip()} ({sid.strip()})</b></div>
                ...
                """, unsafe_allow_html=True)
                st.button("목록 업데이트를 위해 새로고침 하세요", on_click=st.rerun)
...
with tabs[1]:
    ...
    if st.button("내 내역 조회하기", key="btn_lookup"):
        df_l = get_latest_df()
        ...
        if not res.empty:
            ...
            st.warning("🔍 입력하신 정보와 일치하는 예약 내역이 없습니다.")

with tabs[2]:
    st.markdown('<div class="step-header">📋 스터디룸 통합 일정</div>', unsafe_allow_html=True)
    df_v = get_latest_df()
    if not df_v.empty:
        s_date = st.selectbox("📅 조회할 날짜를 선택하세요", sorted(df_v["날짜"].unique()), key="view_date")
        ...
        if not day_df.empty:
            for r_n in ["1번 스터디룸", "2번 스터디룸"]:
                ...
                if room_day.empty:
                    st.caption("해당 날짜에 예약된 일정이 없습니다.")
                else:
...
with tabs[3]:
    ...
    if st.button("연장 가능 여부 확인", key="btn_ext_check"):
        ...
        if not res_e.empty:
            ...
            if (end_dt - timedelta(minutes=30)) <= now_kst < end_dt:
                ...
                st.warning("⚠️ 연장은 종료 30분 전부터 종료 시각까지만 가능합니다.")
        else:
    ...
    if 'ext_target' in st.session_state:
        ...
        if st.button("최종 연장 확정", key="btn_ext_confirm"):
            ...
            st.success("✅ 연장 완료! 15분 내로 QR 재인증을 진행해 주세요.")
...
with tabs[4]:
    ...
    if st.button("조회하여 취소할 내역 찾기", key="btn_can_lookup"):
        df_c = get_latest_df()
    ...
    if 'cancel_list' in st.session_state:
        ...
        target_idx = st.selectbox("취소할 예약 건을 선택하세요", range(len(opts)), format_func=lambda x: opts[x])
        if st.button("✅ 선택한 예약 최종 취소 확정", type="primary"):
